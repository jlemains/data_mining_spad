...
for(i in 1:nb_individu){
  if(data$Domiciliation.de.l.épargne[i] == "de 10 a 100K epargne" 
     || data$Domiciliation.de.l.épargne[i] == "plus de 100K epargne"){
    data$Domiciliation.de.l.épargne[i] <- "plus de 10K epargne"
  }else{
    data$`Domiciliation de l'epargne regroupe`[i] <- data$Domiciliation.de.l.épargne[i]
  }
...
cat_col <- c("Situation.familiale","Domiciliation.de.l.épargne",
            "Domiciliation de l'epargne regroupe",
...
data$Domiciliation.de.l.epargne.regroupe <- relevel(factor(data$`Domiciliation de l'epargne regroupe`), ref = "pas d'épargne")
